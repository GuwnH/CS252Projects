...
def count_words(email_path='data/enron'):
    ...
    for root, ___, files in os.walk(email_path):
        
        for file in files:
            
            file_path = os.path.join(root, file)
            
            with open(file_path, 'r', encoding='latin-1', errors='ignore') as w:
...
def find_top_words(word_freq, num_features=200):
    ...
    for words in sorted_words[:num_features]:
        
        top_words.append(words[0])

    counts = []
    
    for count in sorted_words[:num_features]:
        ...
        if 'ham' in root or 'spam' in root:
            ...
            for file in files:
                ...
                with open(file_path, 'r', encoding='latin-1') as f:
                    ...
                    feature = np.zeros(len(top_words))
                    
                    for i, word in enumerate(top_words):
                        
                        feature[i] = dictionary[word]
                        
                    feats[emails] = feature
...
def make_train_test_sets(features, y, test_prop=0.2, shuffle=True):
    ...
    test_part = np.ceil(test_prop * samples)
    
    test = int(test_part)
    
    y_train = y[test:]
    
    x_train = features[test:]
    
    inds_train = np.arange(test, samples)
    
    y_test = y[:test]
    
    x_test = features[:test]
    
    inds_test = np.arange(test)
