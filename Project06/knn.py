...
class KNN:
    ...
    def predict(self, data, k):
        ...
        for i in range(data.shape[0]):
            ...
            sum_dist = np.sum((self.exemplars - data[i])**2, axis=1)
            
            euc = np.sqrt(sum_dist)
            
            sorted_ind = np.argsort(euc)[:k]
            
            choices = self.classes[sorted_ind]
            
            counts = np.bincount(choices.astype('int64'))
    ...
    def plot_predictions(self, k, n_sample_pts):
        ...
        '''
        color = cartocolors.qualitative.Prism_4.mpl_colors
        
        colors = ListedColormap(color)

        sample_pts = np.linspace(-40, 40, n_sample_pts)
        ...
        xy = np.hstack((x_re, y_re))

        y_pred = self.predict(xy, k)
