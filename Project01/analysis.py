...
class Analysis:
    ...
    def var(self, headers, rows=[]):
        ...
        allX = (data - self.mean(headers, rows))**2
        
        total = np.sum(allX, axis = 0)
        
        sample_Size = data.shape[0]
        
        return total/(sample_Size-1)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        for i, x in enumerate(data_vars):
            ...
            for j, y in enumerate(data_vars):
                ...
                if i == j:
                    ax.hist(self.data.select_data(headers = [x]))
                    ax.set_xlabel(x)
                    ax.set_ylabel(y)
                else:
                    ax.scatter(self.data.select_data(headers = [x]), self.data.select_data(headers = [y]))
                    ax.set_xlabel(x)
                    ax.set_ylabel(y)
                    
        plt.subplots_adjust(top = 4, right = 4)
