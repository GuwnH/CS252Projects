...
class LinearRegression(analysis.Analysis):
    ...
    def linear_regression(self, ind_vars, dep_var, method='scipy', p=1):
        ...
        self.slope = np.array(c[1:]).reshape((len(c[1:]),1))
    ...
    def qr_decomposition(self, A):
        ...
        for i_col in range(A_copy.shape[1]):
            
            temp = np.copy(A_copy[:, i_col])
            
            for j_col in range(i_col):
                
                proj = np.dot(temp, j_col)
                
                temp  = temp - (Q[:,j_col] @ temp) * Q[:,j_col]
    ...
    def predict(self, X=None):
        ...
        if X is None:
            ...
            y = (self.A @ self.slope) + self.intercept
    ...
    def r_squared(self, y_pred):
        ...
        R2: float.
            The R^2 statistic
        '''
        residues = np.sum((self.y - y_pred)**2)
        
        smd = np.sum((self.y - self.y.mean())**2)
        
        self.R2 = 1 - (residues/smd)
    ...
    def compute_mse(self):
        ...
        '''
        divide_value = 1/self.A.shape[0]
        
        error = np.sum((self.residuals)**2)
        
        return divide_value*error
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        '''
        x_orig,_ = super().scatter(ind_var, dep_var, title)
        
        if self.p > 1:
            
            x = np.linspace(x_orig.min(), x_orig.max()).reshape(-1,1)
            
            poly_matrix = self.make_polynomial_matrix(x,self.p)
            
            y = poly_matrix @ self.slope + self.intercept
        
        else:
            x = np.linspace(np.min(self.A), np.max(self.A)).reshape(-1,1)
        
            y = x*self.slope + self.intercept
        
        plt.plot(x, y)
        
        plt.title("R^2: " + str(self.R2) + ". Method by " + title)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), hists_on_diag=True):
        ...
        for i, y in enumerate(data_vars):
            
            for j, x in enumerate(data_vars):
                ...
                if i == j:
                    ...
                    if hists_on_diag == True:
                        ...
                        axis = fig.add_subplot(size, size, i*size+j+1) 
                        
                        axis.hist(self.data.select_data(headers = [x]))
                        
                        axis.set_title(str(x) + " vs. " + str(y))
                        ...
                        ind = np.linspace(self.data.select_data(headers = [x]).min(), self.data.select_data(headers =[x]).max()).reshape(-1,1)
                        
                        dep = (ind @ self.slope) + self.intercept 
                        
                        axis.plot(ind, dep)
                        
                        axis.set_title(str(x) + " vs. " + str(y) + " R^2: " +  str(self.R2))
    ...
    def make_polynomial_matrix(self, A, p):
        ...
        ones = np.ones([A.shape[0], p-1])
        
        A_copy = np.copy(A)
        
        A_copy = np.hstack([A_copy, ones])
        
        for i in range(A.shape[0]):
            
            row_value = A_copy[i,0] 
            
            for j in range(p):
                if j == 0:
                    continue
                else:
                    A_copy[i,j] = row_value**(j+1)
    ...
    def poly_regression(self, ind_var, dep_var, p):
        ...
        self.y = self.data.select_data(dep_vars)
        
        self.p = p
        
        matrix = self.make_polynomial_matrix(self.A, self.p)        
        coeff = self.linear_regression_qr(matrix, self.y)

        self.intercept = float(coeff[0])
        
        self.slope = np.array(coeff[1:]).reshape((len(coeff[1:]),1))
