...
class KMeans:
    ...
    def dist_pt_to_centroids(self, pt, centroids):
        ...
        only a small fraction of the time for your code to stop running)
        '''
        dist = np.linalg.norm(centroids - pt, axis=1)
    ...
    def initialize(self, k):
        ...
        for i in range(k):
            ...
            random_loc = np.random.randint(self.num_samps)
            
            self.centroids[i,:] = self.data[random_loc, :]
    ...
    def initialize_plusplus(self, k):
        ...
        self.centroids[0, :] = self.data[random_loc, :]
        ...
        for i in range(1, k):
            ...
            for j in range(self.data.shape[0]): 
                ...
                dist = self.dist_pt_to_centroids(self.data[j,:], self.centroids)
            ...
            samp = np.random.choice(self.num_samps, p = p)
            
            self.centroids[i, :] = self.data[samp, :]
    ...
    def cluster(self, k=2, tol=1e-2, max_iter=1000, verbose=False, init_function = 'random'):
        ...
        elif init_function == 'random':
            
            self.centroids = self.initialize(k)
        
        while i <= max_iter and np.abs(np.mean(diff)) >= tol:
        ...
        if verbose:
            ...
            print(f"Iterations K-means traversed: " + str(i))
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False, init_method = 'random'):
        ...
        for n in range(n_iter):
            ...
            inertia, i = self.cluster(k, max_iter = n_iter, verbose = verbose, init_function = init_method)
        ...
        min_inertia = np.argmin(inertias)

        self.inertia = inertias[min_inertia]
        
        self.centroids = centroids[min_inertia]

        self.data_centroid_labels = labels[min_inertia]
    ...
    def update_labels(self, centroids):
        ...
        '''
        update = np.ndarray([self.num_samps,])
        ...
        for i in range(self.data.shape[0]):
            ...
            for j in centroids:
                ...
                dist = self.dist_pt_to_pt(self.data[i,:], j)
    ...
    def update_centroids(self, k, data_centroid_labels, prev_centroids):
        ...
        for i in range(k):
            ...
            indices = np.where(data_centroid_labels == i)[0]
            
#             print(indices)
            
            if len(indices) > 0:
                
                new_centroids[i,:] = np.mean(self.data[indices, :], axis = 0)
                
            else:
                
                rand = np.random.randint(self.num_samps)
    ...
    def compute_inertia(self):
        ...
        for i in range(self.data.shape[0]):
            ...
            point2 = self.centroids[int(index), :]
            
            dist = (self.dist_pt_to_pt(point1, point2))**2
            
            total += dist
            
        inertia = total/self.data.shape[0]
    ...
    def plot_clusters(self):
        ...
        '''
        palette = cartocolors.qualitative.Bold_10.mpl_colors

        colors = []
        
        for label in self.data_centroid_labels:
            
            colors.append(palette[int(label)]) 

        plt.scatter(self.data[:, 0], self.data[:, 1], c = colors)

        plt.scatter(self.centroids[:, 0], self.centroids[:, 1], c='black', marker='o', s=100)
    ...
    def replace_color_with_centroid(self):
        ...
        for i in range(self.num_samps):
            ...
            value = int(self.data_centroid_labels[i])
            
            self.data[i, :] = self.centroids[value]
